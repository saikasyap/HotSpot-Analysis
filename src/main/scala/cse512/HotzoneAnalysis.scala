...
object HotzoneAnalysis {
  ...
  def runHotZoneAnalysis(spark: SparkSession, pointPath: String, rectanglePath: String): DataFrame = {
    ...
    // Hot zone analysis - All zones with their count, sorted by "rectangle" string in an ascending order. (coalesce -> to avoid creation of multiple csv files(?))
    val zoneCount = spark.sql("select rectangle, count(point) as points from joinResult group by rectangle order by rectangle asc")
    zoneCount.createOrReplaceTempView("Result")

    return zoneCount.coalesce(1)
